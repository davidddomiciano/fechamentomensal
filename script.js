...
// (Responsável não entra aqui: passou a ser automático, definido por quem marca o checklist)
const TABLE_COLUMNS = [
  "codigo", "filial", "competencia",
...
function criarCardApartirDaLinha(row) {
  ...
  garantirStageLog(card);
  ALL_CHECK_KEYS.forEach(key => {
    if (card.checklist[key]) card.itemLog[key] = { marcadoPor: card.ultimoUsuario, marcadoEm: card.ultimaAtualizacao };
  });
  atualizarProgressaoEtapas(card);
...
function criarLinhaTabela(card, rowIndex) {
  ...
  let html =
    ...
    `<td><div class="cell-readonly" title="Definido automaticamente por quem marca o checklist">${escapeHtml(card.responsavel || "—")}</div></td>` +
    celulaChecklist(card, "pedidosCompra", 3) +
    celulaChecklist(card, "notasDisponiveis", 4) +
    celulaChecklist(card, "pendenciasFiscais", 5) +
    celulaChecklist(card, "notasLancadas", 6) +
    celulaChecklist(card, "reclassificacao", 7) +
    celulaChecklist(card, "conferencia", 8) +
    celulaChecklist(card, "provisoes", 9) +
    celulaChecklist(card, "impostos", 10) +
    celulaChecklist(card, "contabilizacaoFinal", 11) +
...
function atualizarLinhaComputada(id) {
  ...
  if (linha) {
    const cellResponsavel = linha.querySelector(".cell-readonly");
