...
// Gerada automaticamente a partir de ALL_CHECK_KEYS (definido em script.js) —
// ao adicionar/remover um item do checklist em STAGES, esta lista se ajusta
// sozinha. As 4 primeiras e as 4 últimas posições são fixas.
const GRAPH_COLUNAS = [
...
async function prepararPlanilha() {
  try {
    ...
    if (!tabelaExiste) {
      const ultimaColuna = String.fromCharCode(65 + GRAPH_COLUNAS.length - 1); // A=65 (funciona até 26 colunas / Z)
    ...
    // Coluna "Última Atualização" (a 3ª depois dos itens de checklist) como texto — evita conversão automática de data
    const indiceColunaData = GRAPH_COLUNAS.indexOf("ultimaAtualizacao");
    const colunaData = String.fromCharCode(65 + indiceColunaData);
...
function cardParaLinha(card) {
  return [
    ...
    ...ALL_CHECK_KEYS.map(key => (card.checklist[key] ? "Sim" : "Não")),
    card.manualStage || "",
    card.ultimaAtualizacao || "",
    card.ultimoUsuario || "",
    JSON.stringify({
      stageLog: card.stageLog, itemLog: card.itemLog, historico: card.historico,
...
function linhaParaCard(linha, idExistente) {
  const [codigo, filial, competencia, responsavel] = linha;
  const valoresChecklist = linha.slice(4, 4 + ALL_CHECK_KEYS.length);
  const [manualStage, ultimaAtualizacao, ultimoUsuario, dadosInternosStr] = linha.slice(4 + ALL_CHECK_KEYS.length);

  let internos = {};
  try { internos = JSON.parse(dadosInternosStr || "{}"); } catch (e) { internos = {}; }

  const checklist = {};
  ALL_CHECK_KEYS.forEach((key, i) => { checklist[key] = valorVerdadeiro(valoresChecklist[i]); });
...
async function carregarDeGraph(manual) {
  ...
  try {
    ...
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ cards: state.cards, sort: state.sort, view: state.view, slaTargets: state.slaTargets, historicoMeses: state.historicoMeses }));
    renderizarTudo();
